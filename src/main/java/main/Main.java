...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Depth 1: " + dataAccessor.testQuery("SELECT u1.node_id, u1.name, u1.job, u1.birthday FROM t_user u1 JOIN t_endorsement e1 ON u1.node_id = e1.target_node_id INNER JOIN (SELECT node_id FROM t_user ORDER BY RAND() LIMIT 20) AS i ON e1.source_node_id = i.node_id;"));
        System.out.println("Depth 2: " + dataAccessor.testQuery("SELECT u2.node_id, u2.name, u2.job, u2.birthday FROM t_user u2 JOIN t_endorsement e2 ON u2.node_id = e2.target_node_id WHERE e2.source_node_id IN (SELECT u1.node_id FROM t_user u1 JOIN t_endorsement e1 ON u1.node_id = e1.target_node_id INNER JOIN (SELECT node_id FROM t_user ORDER BY RAND() LIMIT 20) AS i ON e1.source_node_id = i.node_id);"));
